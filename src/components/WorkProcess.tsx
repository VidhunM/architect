          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10">
            {stepsA.map((step, idx) => (
              <div
                ...
                style={{ animationDelay: `${idx * 150}ms` }}
              >
                <div className="w-12 h-12 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-slate-700 text-white flex items-center justify-center text-lg sm:text-sm md:text-base font-bold shadow mb-3 sm:mb-3 leading-none">
                  {step.number}
                </div>
                <img
                  src={step.imgSrc}
                  alt={step.title}
                  className="w-48 sm:w-32 md:w-[180px] h-auto object-contain select-none transition-transform duration-300 ease-out group-hover:-translate-y-1"
                  draggable={false}
                />
                <h3 className="mt-4 sm:mt-4 text-lg sm:text-sm md:text-xl font-semibold text-blue-900">
                  {step.title}
                </h3>
              </div>
            ))}
